...
def main(config):
    ...
    # Check if GPU is available for sync_bn
    use_sync_bn = config.GPU >= 0 and torch.cuda.is_available() and torch.cuda.device_count() > 0
    ...
    model = networks.UNet(
        ...
        up_mode="upsample",
        with_tanh=False,
        sync_bn=use_sync_bn,  # Only use sync_bn when GPU is available
        antialiasing=True,
    )
