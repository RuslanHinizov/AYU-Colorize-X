...
export default function PhotoEditor() {
    ...
    const handleDownload = async () => {
        if (!jobId) return;
        try { const response = await axios.get(`/jobs/${jobId}/download`, { responseType: 'blob' }); const url = window.URL.createObjectURL(new Blob([response.data])); const link = document.createElement('a'); link.href = url; link.setAttribute('download', `colorized_${jobId}.jpg`); document.body.appendChild(link); link.click(); document.body.removeChild(link); } catch (err) { console.error('İndirme başarısız:', err); }
    };
        ...
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 p-6 lg:p-8">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-2"><Aperture className="w-5 h-5 text-primary" /><span className="font-display text-lg">Fotoğraf Renklendirme</span></div>
                </motion.div>
                        ...
                        <div className="card group">
                            ...
                            <div className="relative">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div><h2 className="text-lg font-semibold">AI Modeli</h2><p className="text-sm text-muted">Renklendirme türü</p></div>
                                </div>
                                <div className="grid grid-cols-2 gap-3">
                                    {[{ id: 'artistic', name: 'Artistic', desc: 'Canlı renkler', icon: Palette }, { id: 'stable', name: 'Stable', desc: 'Doğal tonlar', icon: ImageIcon }].map((m) => (
                        ...
                        <div className="card min-h-[400px] max-h-[calc(100vh-100px)] flex flex-col relative overflow-hidden group">
                            ...
                            {!preview ? (
                                <label role="button" aria-label="Fotoğraf yükle" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && e.currentTarget.querySelector('input')?.click()} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} className="relative flex-1 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-primary/40 hover:bg-primary/5 transition-all duration-500 group/upload m-2">
                                    ...
                                    <p className="text-muted mb-4 text-center max-w-xs">Siyah beyaz fotoğraflarınızı AI ile renklendirin</p>
                                            ...
                                            {result ? (
                                                viewMode === 'slider' ? <div className="absolute inset-0 w-full h-full"><BeforeAfterSlider original={preview} modified={result} /></div> : (
                                                    <div className="grid grid-cols-2 gap-3 w-full p-3 items-center h-full overflow-auto">
                                                        ...
                                                        <div className="relative"><span className="absolute top-3 left-3 bg-gradient-to-r from-primary to-secondary px-3 py-1.5 rounded-lg text-xs text-white z-10">Renkli</span><img src={result} alt="Colorized" className="w-full h-auto rounded-xl shadow-2xl ring-2 ring-primary/20" /></div>
                                                    </div>
                                                )
                                            ...
                                            <AnimatePresence>
                                                {isProcessing && (
                                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/80 backdrop-blur-xl flex flex-col items-center justify-center z-30">
                                                        ...
                                                        <h3 className="text-2xl font-display font-semibold mb-2 text-gradient">AI Renklendirme</h3>
                                                        <p className="text-muted mb-6">{progress < 30 ? 'Analiz ediliyor...' : progress < 70 ? 'Renklendiriliyor...' : 'Tamamlanıyor...'}</p>
