...
export default function RestorePage() {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            setError(typeof detail === 'string' ? detail : (Array.isArray(detail) ? t('editor.invalidData') : t('editor.processingFailed'))); setIsProcessing(false);
        }
    };
        ...
        <div className="min-h-screen relative overflow-hidden">
            ...
            <div className="relative z-10 p-6 lg:p-8">
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between mb-8">
                    ...
                    <div className="flex items-center gap-2"><Wand2 className="w-5 h-5 text-accent" /><span className="font-display text-lg">{t('editor.restorePageTitle')}</span></div>
                </motion.div>
                        ...
                        <div className="card group">
                            ...
                            <div className="relative">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div><h2 className="text-lg font-semibold">CodeFormer AI</h2><p className="text-sm text-muted">{t('editor.restoreAiSubtitle')}</p></div>
                                </div>
                                <div className="space-y-3 text-sm text-muted">
                                    <div className="flex items-start gap-2">
                                        <Sparkles className="w-4 h-4 text-accent mt-0.5 shrink-0" />
                                        <span>{t('editor.restoreFeature1')}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                        <Sparkles className="w-4 h-4 text-accent mt-0.5 shrink-0" />
                                        <span>{t('editor.restoreFeature2')}</span>
                                    </div>
                                    <div className="flex items-start gap-2">
                                        <Sparkles className="w-4 h-4 text-accent mt-0.5 shrink-0" />
                                        <span>{t('editor.restoreFeature3')}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="card min-h-[400px] max-h-[calc(100vh-100px)] flex flex-col relative overflow-hidden group">
                            ...
                            {!preview ? (
                                <label role="button" aria-label={t('editor.uploadPhotoTitle')} tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && e.currentTarget.querySelector('input')?.click()} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()} className="relative flex-1 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-accent/40 hover:bg-accent/5 transition-all duration-500 group/upload m-2">
                                    ...
                                    <p className="text-muted mb-4 text-center max-w-xs">{t('editor.restoreUploadDesc')}</p>
                                            ...
                                            <AnimatePresence>
                                                {isProcessing && (
                                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/80 backdrop-blur-xl flex flex-col items-center justify-center z-30">
                                                        ...
                                                        <h3 className="text-2xl font-display font-semibold mb-2 text-gradient">{t('editor.restoreProcessingTitle')}</h3>
                                                        <p className="text-muted mb-6">{progress < 30 ? t('editor.analyzing') : progress < 70 ? t('editor.restoringFaces') : t('editor.finishing')}</p>
