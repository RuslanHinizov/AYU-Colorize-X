...
export default function VideoEditor() {
    ...
    const handleFileSelect = (e) => {
        ...
        if (file) {
            if (!file.type.startsWith('video/')) { setError(t('editor.videoOnlyError')); return; }
                ...
                <div className="grid lg:grid-cols-12 gap-6 lg:gap-8">
                        ...
                        <div className="card group">
                            ...
                            <div className="relative">
                                <div className="flex items-center gap-4 mb-6">
                                    ...
                                    <div><h2 className="text-lg font-semibold">{t('editor.videoAiTitle')}</h2><p className="text-sm text-muted">{t('editor.videoAiDesc')}</p></div>
                                </div>
                                <div className="p-4 rounded-xl bg-secondary/5 border border-secondary/20">
                                    <p className="text-sm text-muted">{t('editor.videoModelDesc')}</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="lg:col-span-8 xl:col-span-9">
                        <div className="card min-h-[400px] max-h-[calc(100vh-100px)] flex flex-col relative overflow-hidden group">
                            ...
                            {!preview ? (
                                <label role="button" aria-label={t('editor.uploadVideoTitle')} tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && e.currentTarget.querySelector('input')?.click()} className="relative flex-1 border-2 border-dashed border-white/10 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-secondary/40 hover:bg-secondary/5 transition-all duration-500 group/upload m-2">
                                    ...
                                    <h3 className="text-2xl font-display font-semibold mb-3 text-gradient">{t('editor.videoTitle')}</h3>
                                    <p className="text-muted mb-4 text-center max-w-xs">{t('editor.uploadVideoDesc')}</p>
                                            ...
                                            {result ? (
                                                viewMode === 'slider' ? <div className="absolute inset-0 w-full h-full"><VideoBeforeAfterSlider original={preview} modified={result} /></div> : (
                                                    <div className="grid grid-cols-2 gap-3 w-full h-full p-3">
                                                        <div className="relative"><span className="absolute top-3 left-3 bg-black/70 backdrop-blur-sm px-3 py-1.5 rounded-lg text-xs text-white z-10">{t('editor.original')}</span><video src={preview} controls className="w-full h-full object-contain rounded-xl" /></div>
                                                        <div className="relative"><span className="absolute top-3 left-3 bg-gradient-to-r from-secondary to-accent px-3 py-1.5 rounded-lg text-xs text-white z-10">{t('editor.colorized')}</span><video src={result} controls className="w-full h-full object-contain rounded-xl" /></div>
                                                    </div>
                                                )
                                            ...
                                            <AnimatePresence>
                                                {isProcessing && (
                                                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-background/80 backdrop-blur-xl flex flex-col items-center justify-center z-30">
                                                        ...
                                                        <h3 className="text-2xl font-display font-semibold mb-2 text-gradient">{t('editor.videoTitle')}</h3>
